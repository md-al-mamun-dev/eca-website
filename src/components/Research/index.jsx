...
const Research = ({showHeading}) => {
  return (
    <div id='research' className={` ${styles.container} ${styles.research}`}>
      {
        showHeading 
        ? <h2 className={`${styles.title} f-semi-bold`}>Research</h2>
        : <></>
        }
      ...
      {/* <div className={` ${styles.research_papers}`} >

        {
            researches.map(research_paper =>( 
              <div className={`${styles.research_paper_item}`}>
                <div className={`${styles.thumbnail_wrapper}`}>
                  <a href={`/research/${research_paper.id}`}>
                    {
                      research_paper.thumbnail !='' 
                            ?<Image className={`${styles.thumbnail_img}`} src={`/images/thumbnails/${research_paper.thumbnail}`} fill/> 
                            : <div className={`${styles.blank_div}`}>
                                <h1 className={`${styles.alternative_title}`}>{research_paper.name}</h1>
                              </div>
                    }
